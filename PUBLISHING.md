...
1. Create a new **empty** repo on GitHub (no README/license — this folder already has them). Name suggestion: `cardamom`.
2. In a terminal, from this folder, run:

   ```bash
   git remote add origin https://github.com/<your-username>/cardamom.git
   git branch -M main
...
> `gh repo create cardamom --public --source=. --push`
...
2. **Upload the package:** click *Add new item* and upload `cardamom.zip` (already built in this folder).
...
- [ ] Confirm `cardamom.zip` opens to the files at its root (manifest.json at top level, not inside a subfolder).
